...
Being upfront about what's incomplete against the brief:

- **Payroll/Accounting integration point.** The brief requires Fleet to expose a
  controlled integration surface (e.g. driver bonus on trip completion, cost data for
  accounting) without directly writing into those modules' tables. Not yet built.
- **Contract-based trip amount calculation.** The brief describes trip amount as
  calculated from "the applicable contract." There is no `Contract` entity yet — amount
  is entered manually on the trip.
- **No seed script.** `backend/package.json` has a `prisma:seed` script wired up, but
  `prisma/seed.ts` doesn't exist yet, so there's no one-command way to load demo data.
- **No vehicle-status history log.** `Vehicle.status` is a plain current-value column,
  not a time-bounded/append-only table like assignments and maintenance status — a
  deliberate scope decision, since the brief only requires auditable history for
  maintenance status specifically, not general vehicle status.

## Critical engineering questions (from the assessment brief)

- **Source of truth for the current driver?** The open (`effectiveTo IS NULL`) row in
  `vehicle_assignments` — never a column on `Vehicle`.
- **How are historical assignments preserved?** Reassignment closes the old row rather
  than deleting or overwriting it; nothing is ever destructively updated.
- **How is an inactive vehicle prevented from being used?** `Vehicle.status` is checked
  at every write path that resolves a vehicle for an operational action
  (`findActiveVehicleOrThrow`), not just filtered out of a dropdown in the UI.
- **How are documents and expiry modeled?** `vehicle_documents.expiry_date`, with
  expiring/expired documents computed fresh from that column on every request — not a
  separately stored alert that could go stale.
- **How is vehicle operating cost calculated?** The Vehicle Profile endpoint sums
  `maintenance_records.cost` and `fuel_records.total_cost` server-side using `Decimal`
  arithmetic (never floating point), never trusting a client-supplied total.
- **How should Fleet communicate with Payroll without tight coupling?** Not yet
  implemented — see Known gaps above.
- **How is duplicate trip processing prevented?** A unique constraint on
  `(companyId, idempotencyKey)` on the `trips` table; a repeated submission with the same
  key is rejected with a clean 409, not a raw database error.
